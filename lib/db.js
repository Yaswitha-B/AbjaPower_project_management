...
export async function getSightings(issueId) {
  return q('SELECT * FROM sightings WHERE issue_id = $1 ORDER BY date', [issueId]);
}
...
export async function getAllIssues() {
  return q(`
    SELECT ia.*, p.name AS project_name, i.verified_at, i.verified_by
    FROM issue_age ia
    JOIN issues i ON i.id = ia.id
...
export async function patchIssue(id, data) {
  const { description, owner, owner_type, stage, waiting_on, note, resolved_date, recur, raised_date } = data;
  const rows = await q(`
    UPDATE issues SET
  ...
  `, [
    ...
    recur ?? null, raised_date ?? null, id,
  ]);
  return rows[0] ?? null;
}
...
export async function getUnverifiedIssues(projectId) {
  if (projectId) {
    return q(`
      SELECT ia.*, p.name AS project_name, i.verified_at, i.verified_by, i.verify_flag
      FROM issue_age ia
      JOIN issues i ON i.id = ia.id
      JOIN projects p ON p.id = ia.project_id
