...
// System-wide audit trail. Deliberately never throws — a logging failure must
// never block the primary action (saving an MIS row, updating an issue, etc).
export async function logActivity({ actor, actor_role, action, entity_type, entity_id, project_id, detail }) {
  try {
    await q(`
      INSERT INTO activity_log (actor, actor_role, action, entity_type, entity_id, project_id, detail)
      VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb)
    `, [
      actor ?? null, actor_role ?? 'reporter', action, entity_type,
      entity_id != null ? String(entity_id) : null, project_id ?? null,
      JSON.stringify(detail ?? {}),
    ]);
  } catch (err) {
    console.error('logActivity failed (non-fatal)', action, err.message);
  }
}

// Our own allowlist — Neon Auth verifying an email is not the same as us
// authorizing it. A verified email with no row here gets no access.
export async function getUserRole(email) {
  const rows = await q('SELECT email, name, role, project_ids FROM user_roles WHERE email = $1', [email.toLowerCase()]);
  return rows[0] ?? null;
}

export async function getAllUserRoles() {
  return q('SELECT email, name, role, project_ids, created_at FROM user_roles ORDER BY created_at DESC');
}

// project_ids: [] (default) means "all projects" — the empty-vs-populated
// distinction is what read.js/write.js check against on every project-scoped
// request. project_ids is optional here: pass undefined/null to leave an
// existing user's scope untouched (e.g. a role-only change), or an array to
// set it explicitly. Without this distinction, changing someone's role alone
// would silently reset their project scope back to "all" every time.
export async function upsertUserRole({ email, name, role, project_ids }) {
  const rows = await q(`
    INSERT INTO user_roles (email, name, role, project_ids)
    VALUES ($1, $2, $3, COALESCE($4::jsonb, '[]'::jsonb))
    ON CONFLICT (email) DO UPDATE
      SET name = EXCLUDED.name, role = EXCLUDED.role,
          project_ids = COALESCE($4::jsonb, user_roles.project_ids)
    RETURNING *
  `, [email.toLowerCase(), name ?? null, role, project_ids != null ? JSON.stringify(project_ids) : null]);
  return rows[0];
}

export async function deleteUserRole(email) {
  const rows = await q('DELETE FROM user_roles WHERE email = $1 RETURNING *', [email.toLowerCase()]);
  return rows[0] ?? null;
}

// Lightweight lookups used only to resolve which project an existing
// issue/mis_row belongs to, for project-scope enforcement — not the full row.
export async function getIssueProjectId(id) {
  const rows = await q('SELECT project_id FROM issues WHERE id = $1', [id]);
  return rows[0]?.project_id ?? null;
}

export async function getMisRowProjectId(id) {
  const rows = await q('SELECT project_id FROM mis_rows WHERE id = $1', [id]);
  return rows[0]?.project_id ?? null;
}
...
export async function getActivityBreakdown(projectId) {
  return q(`
    SELECT act->>'activity' AS activity, SUM(m.hr_total) AS man_days
...
export async function getTowerBreakdown(projectId) {
  return q(`
    SELECT act->>'tower' AS tower, SUM(m.hr_total) AS man_days
    FROM mis_rows m, LATERAL jsonb_array_elements(m.activities) AS act
    WHERE m.project_id = $1
    GROUP BY act->>'tower'
    ORDER BY man_days DESC
...
// Single source of truth for HR totals: zero-valued roles are never stored,
// and the total is always the sum of what's left — callers cannot override either.
function cleanHrDetail(detail) {
  return Object.fromEntries(Object.entries(detail ?? {}).filter(([, v]) => Number(v) > 0));
}
function sumHr(detail) {
  return Object.values(detail).reduce((sum, v) => sum + Number(v), 0);
}

export async function insertMisRow(data) {
  const { project_id, date, package: pkg, hr_detail, activities, reported_by, source } = data;
  const detail = cleanHrDetail(hr_detail);
  const total  = sumHr(detail);
  const rows = await q(`
    INSERT INTO mis_rows (project_id, date, package, hr_total, hr_detail, activities, reported_by, source)
    VALUES ($1, $2, $3, $4, $5::jsonb, $6::jsonb, $7, $8)
    ON CONFLICT (project_id, date, package) DO UPDATE
      SET hr_total   = EXCLUDED.hr_total,
          hr_detail  = EXCLUDED.hr_detail,
          activities = EXCLUDED.activities,
          reported_by = EXCLUDED.reported_by
    RETURNING *
  `, [project_id, date, pkg, total,
      JSON.stringify(detail), JSON.stringify(activities),
...
export async function createProject(data) {
  const { id, name, discipline, id_prefix, towers, work_packages, activity_types, hr_roles,
          status, start_date, end_date } = data;
  const rows = await q(`
    INSERT INTO projects
      (id, name, discipline, id_prefix, towers, work_packages, activity_types, hr_roles,
...
export async function updateProject(id, data) {
  const { name, discipline, status, start_date, end_date, towers, work_packages, activity_types, hr_roles } = data;
  const rows = await q(`
    UPDATE projects SET
      name           = $1,
      discipline     = $2,
...
export async function updateMisRow(id, data) {
  const { date, package: pkg, hr_detail, activities, reported_by } = data;
  // Only recompute the total when hr_detail is actually part of this patch —
  // a patch that only touches date/activities must leave hr_total untouched.
  const detail = hr_detail != null ? cleanHrDetail(hr_detail) : null;
  const total  = detail != null ? sumHr(detail) : null;
  const rows = await q(`
    UPDATE mis_rows SET
      date        = COALESCE($1, date),
      package     = COALESCE($2, package),
      hr_total    = COALESCE($3, hr_total),
      hr_detail   = COALESCE($4::jsonb, hr_detail),
      activities  = COALESCE($5::jsonb, activities),
      reported_by = COALESCE($6, reported_by)
    WHERE id = $7
    RETURNING *
  `, [
    date ?? null, pkg ?? null, total,
